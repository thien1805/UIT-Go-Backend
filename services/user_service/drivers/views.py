...
def register_driver_profile(request):
    ...
    return Response({
        ...
        'error': {
            ...
            'message': 'Đăng kí thông tin tài xế không thành công',
...
def get_driver_profile_by_id(request, driver_id):
    ...
    lấy thông tin tài xế bằng ID (API nội bộ được bảo mật bằng API Gateway)
    """
    try:
        driver_profile = DriverProfile.objects.select_related('user').get(id=driver_id)
    
        #Nếu request từ service khác, gọi API Gateway để lấy thông tin tài xế
        service_token = request.headers.get('X-Service-Token')
        if service_token == getattr(settings, 'INTERNAL_SERVICE_TOKEN', None):
            serializer = DriverProfileSerializer(driver_profile)
        else:
            serializer = DriverPublicSerializer(driver_profile)
        return Response({
            'success': True,
            'data': {
                serializer.data
...
def update_driver_status(request):
    ...
    return Response({
        ...
        'error': {
            'code': 'VALIDATION_ERROR',
            'message': 'Cập nhật trạng thái online không thành công',
            'details': serializer.errors
        }
    }, status=status.HTTP_400_BAD_REQUEST)
...
def update_driver_stats(request, driver_id):
    ...
    return Response({
        ...
        'error': {
            'code': 'VALIDATION_ERROR',
            'message': 'Cập nhật thống kê của tài xế không thành công',
            'details': serializer.errors
        }
    }, status=status.HTTP_400_BAD_REQUEST)